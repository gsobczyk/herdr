...
#[derive(Clone, Copy)]
enum PaneBorderAction {
    ToggleZoom,
    Close,
}

impl ClientShellState {
    /// Zoom and close icons the server draws on a pane's top border. Their
    /// placement is derived from the pane rect, so no extra surface data is needed.
    fn pane_action_at(&self, point: (u16, u16)) -> Option<(String, PaneBorderAction)> {
        self.hits.panes.iter().find_map(|hit| {
            let has_top_border = hit.inner_rect.y > hit.rect.y;
            let layout = crate::ui::pane_action_layout(hit.rect, has_top_border, false)?;
            if super::contains(layout.zoom_rect, point) {
                Some((hit.pane_id.clone(), PaneBorderAction::ToggleZoom))
            } else if super::contains(layout.close_rect, point) {
                Some((hit.pane_id.clone(), PaneBorderAction::Close))
            } else {
                None
            }
        })
    ...
    pub(super) fn handle_mouse(&mut self, mouse: MouseEvent, outcome: &mut ClientShellInput) {
        ...
        match mouse.kind {
            ...
            MouseEventKind::Down(MouseButton::Left) => {
                ...
                if let Some((pane_id, action)) = self.pane_action_at(point) {
                    self.mode = ClientShellMode::Terminal;
                    self.push_endpoint_method(
                        crate::api::schema::Method::PaneFocus(crate::api::schema::PaneTarget {
                            pane_id: pane_id.clone(),
                    ...
                    let method =
                        match action {
                            PaneBorderAction::ToggleZoom => crate::api::schema::Method::PaneZoom(
                                crate::api::schema::PaneZoomParams {
                                    pane_id: Some(pane_id),
                                    mode: crate::api::schema::PaneZoomMode::Toggle,
                                },
                            ),
                            PaneBorderAction::Close => crate::api::schema::Method::PaneClose(
