...
/// Zoom and close icon placement on a pane's top border. Server rendering and
/// client hit testing both derive it from the pane rect so it stays in sync.
pub(crate) struct PaneActionLayout {
    pub(crate) zoom_rect: Rect,
    pub(crate) close_rect: Rect,
    zoom_label: &'static str,
}

pub(crate) fn pane_action_layout(
    rect: Rect,
    has_top_border: bool,
    zoomed: bool,
) -> Option<PaneActionLayout> {
    if !has_top_border || rect.width <= 6 {
        return None;
    }
    ...
    let zoom_w = display_width(zoom_label).clamp(1, u16::MAX as usize) as u16;
    let close_w = display_width(PANE_ACTION_CLOSE).clamp(1, u16::MAX as usize) as u16;
    let gap_w: u16 = 1;
    let required = zoom_w.saturating_add(gap_w).saturating_add(close_w);
    if rect.width.saturating_sub(2) < required {
        return None;
    }

    let right_inner = rect.x.saturating_add(rect.width).saturating_sub(2);
    ...
    Some(PaneActionLayout {
        zoom_rect: Rect::new(zoom_x, rect.y, zoom_w, 1),
        close_rect: Rect::new(close_x, rect.y, close_w, 1),
        zoom_label,
    })
}
...
fn render_pane_border_titles(
    ...
    for info in pane_infos {
        ...
        let layout = pane_action_layout(info.rect, info.borders.contains(Borders::TOP), ws.zoomed);
        ...
        if let Some(title) = title {
            ...
            if let Some(layout) = &layout {
                end_x = end_x.min(layout.zoom_rect.x.saturating_sub(1));
            }
            if start_x < end_x {
                buf.set_stringn(
                    start_x,
...
mod tests {
    ...
    #[test]
    fn pane_action_icons_sit_at_top_right_and_leave_the_title_room() {
        ...
        terminal_state.set_manual_label("a-very-long-pane-label".into());
        app.terminals.insert(terminal_id, terminal_state);

        let layout = pane_action_layout(Rect::new(0, 0, 20, 5), true, false).expect("layout");
        assert_eq!(layout.zoom_rect.y, 0);
        assert_eq!(
            layout.close_rect.x + layout.close_rect.width,
            19,
            "close icon ends inside the right border"
        );
        assert_eq!(
            layout.close_rect.x,
            layout.zoom_rect.x + layout.zoom_rect.width + 1
        );
        assert!(pane_action_layout(Rect::new(0, 0, 20, 5), false, false).is_none());
        assert!(pane_action_layout(Rect::new(0, 0, 6, 5), true, false).is_none());
        ...
        assert_eq!(buffer[(layout.close_rect.x + 1, 0)].symbol(), "\u{2715}");
        let before_icons = buffer[(layout.zoom_rect.x - 1, 0)].symbol();
        assert!(
            before_icons == " " || before_icons == "\u{2500}",
            "title must not run into the icons, found {before_icons:?}"
        );
    }
