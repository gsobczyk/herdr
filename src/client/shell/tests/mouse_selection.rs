...
#[test]
fn clicking_pane_border_icons_zooms_and_closes_the_pane() {
    let mut bordered = surface();
    bordered.panes[0].rect = SurfaceRect {
        x: 0,
        y: 0,
        width: 20,
        height: 5,
    };
    bordered.panes[0].inner_rect = SurfaceRect {
        x: 1,
        y: 1,
        width: 18,
        height: 3,
    ...
    state.compose(106, 20).expect("pane frame");
    let pane = state.hits.panes[0].clone();
    let layout = crate::ui::pane_action_layout(pane.rect, true, false).expect("icon layout");

    let click = |state: &mut ClientShellState, rect: ratatui::layout::Rect| {
        state
            .handle_raw_events(vec![RawInputEvent::Mouse(MouseEvent {
                kind: MouseEventKind::Down(MouseButton::Left),
                column: rect.x,
                row: rect.y,
                modifiers: KeyModifiers::empty(),
    ...
    let zoom = click(&mut state, layout.zoom_rect);
    assert!(
        matches!(
            &zoom[..],
            [
                crate::api::schema::Method::PaneFocus(focus),
                crate::api::schema::Method::PaneZoom(params)
            ] if focus.pane_id == "pane_1"
                && params.pane_id.as_deref() == Some("pane_1")
                && params.mode == crate::api::schema::PaneZoomMode::Toggle
        ),
        "unexpected zoom methods: {zoom:?}"
    );

    let close = click(&mut state, layout.close_rect);
    assert!(
        matches!(
            &close[..],
            [
            ...
            ] if focus.pane_id == "pane_1" && target.pane_id == "pane_1"
        ),
        "unexpected close methods: {close:?}"
    );
}
